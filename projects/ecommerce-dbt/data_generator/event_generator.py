...
class EventGenerator:
    ...
    def generate_order(self, timestamp: Optional[datetime] = None) -> Order:
        ...
        quantity = random.choices(
            [1, 2, 3, 4, 5],
            weights=[0.6, 0.2, 0.1, 0.05, 0.05]  # Most orders are single item
        )[0]
    ...
    def generate_page_view(
        ...
        page_type = random.choices(
            page_types,
            weights=[0.3, 0.3, 0.3, 0.05, 0.05]  # Most views are browsing
        )[0]
    ...
    def generate_batch(
        ...
        for i in range(num_orders):
            timestamp = start_time + timedelta(
                seconds=random.uniform(0, duration_seconds)
            )
