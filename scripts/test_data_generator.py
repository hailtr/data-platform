...
from data_generator.user_generator import UserGenerator  # noqa: E402
from data_generator.product_generator import ProductGenerator  # noqa: E402
...
def test_product_generator():
    ...
    for i, product in enumerate(product_gen.products[:5]):
        print(
            f"  {product.product_id}: {product.category}, ${product.price:.2f}, {product.supplier}"
        )
...
def test_event_generator():
    ...
    config = GeneratorConfig(
        orders_per_second=1.0, page_views_per_second=5.0, total_users=100, total_products=50
    )
...
def test_batch_generation():
    ...
    if orders:
        print(
            f"\nFirst order: {orders[0].order_id} at " f"{orders[0].timestamp.strftime('%H:%M:%S')}"
        )
        print(
            f"Last order: {orders[-1].order_id} at " f"{orders[-1].timestamp.strftime('%H:%M:%S')}"
        )

    if page_views:
        print(
            f"\nFirst page view: {page_views[0].view_id} at "
            f"{page_views[0].timestamp.strftime('%H:%M:%S')}"
        )
        print(
            f"Last page view: {page_views[-1].view_id} at "
            f"{page_views[-1].timestamp.strftime('%H:%M:%S')}"
        )
