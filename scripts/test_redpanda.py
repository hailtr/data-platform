...
from shared.messaging import RedpandaProducer, RedpandaConsumer  # noqa: E402
...
def test_producer():
    ...
    try:
        ...
        success = producer.publish(
            topic=settings.KAFKA_TOPIC_ORDERS, event=test_event, key="test_001"
        )
        ...
        batch_events = [
            {"test_id": f"test_{i:03d}", "message": f"Test message {i}"} for i in range(1, 6)
        ]
...
def test_consumer():
    ...
    try:
        ...
        consumer.consume(handler=message_handler, max_messages=10, timeout_ms=5000)
